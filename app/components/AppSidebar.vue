<template>
  <div>
    ...
    <Transition name="fade">
      <div v-if="isOpen" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 md:hidden" aria-hidden="true"
        @click="$emit('close')" />
    </Transition>
    ...
    <aside
      ...
      <!-- Brand Header -->
      <div
        class="h-16 sm:h-20 flex items-center justify-between px-4 sm:px-6 border-b border-[#f8f5ee]/10 flex-shrink-0">
        <NuxtLink to="/" class="flex items-center gap-2.5 sm:gap-3 group min-w-0">
          <div
            class="p-2 rounded-xl bg-[#c9793f]/10 group-hover:bg-[#c9793f]/20 transition-colors flex-shrink-0 overflow-hidden w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center">
            <img v-if="globalSettings.logoUrl" :src="globalSettings.logoUrl" alt="Logo Toko"
              class="w-full h-full object-cover rounded-xl" />
            <LucideCoffee v-else class="w-6 h-6 sm:w-7 sm:h-7 text-[#c9793f]" />
          </div>
          <div class="flex flex-col min-w-0">
            <span class="font-bold text-sm sm:text-base tracking-tight text-[#f8f5ee] truncate">
              {{ globalSettings.shopName.toUpperCase() }}
            </span>
            <span
              class="text-[10px] sm:text-[11px] font-medium text-[#f8f5ee]/60 -mt-0.5 tracking-wider uppercase truncate">
              Point of Sale
            </span>
          </div>
        </NuxtLink>
        ...
        <button
          ...
          aria-label="Tutup Menu" @click="$emit('close')">
      ...
      <!-- Navigasi -->
      <nav
        class="flex-1 min-h-0 overflow-y-auto px-3 sm:px-4 py-6 sm:py-8 space-y-6 sm:space-y-8 scrollbar-thin scrollbar-thumb-[#f8f5ee]/10 scrollbar-track-transparent">
        ...
        <template v-else>
          <div v-for="group in menuGroups" :key="group.label" class="space-y-2">
            ...
            <NuxtLink v-for="item in group.items" :key="item.to" :to="item.to"
              ...
              <div v-if="isActive(item.to)" class="ml-auto w-1.5 h-1.5 rounded-full bg-[#1c1410]/30 flex-shrink-0">
              </div>
            </NuxtLink>
          </div>
        </template>
        ...
        <div class="flex items-center gap-3 px-3 py-3 rounded-2xl bg-[#f8f5ee]/5 border border-[#f8f5ee]/10">
          ...
          <template v-else>
            <div class="relative flex-shrink-0">
              <img
                :src="`https://api.dicebear.com/8.x/notionists-neutral/svg?seed=${globalUser.name || 'user'}&backgroundColor=c9793f`"
                alt="Avatar" class="w-10 h-10 rounded-full bg-[#1c1410] border-2 border-[#f8f5ee]/10" />
              <span class="absolute bottom-0 right-0 block h-3 w-3 rounded-full ring-2 ring-[#17100d] bg-emerald-500" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-bold text-white truncate">
                {{ globalUser.name || 'Memuat...' }}
              </p>
...
import { useRoute, useRequestHeaders, useFetch, useState, resolveComponent } from '#imports'
...
const globalUser = useState('global-auth-user', () => ({
  name: '',
  role: '' as 'KASIR' | 'PEMILIK' | ''
}))
...
const { data: userData, pending: userPending } = useFetch<AuthUser>('/api/auth/me', {
  key: 'auth-user',
  headers,
  onResponseError: () => {},
})
...
watchEffect(() => {
  if (settingsData.value?.data) {
    globalSettings.value.shopName = settingsData.value.data.shop_name
    globalSettings.value.logoUrl = settingsData.value.data.logo_url
  }
})

watchEffect(() => {
  if (userData.value) {
    globalUser.value.name = userData.value.name
    globalUser.value.role = userData.value.role
  }
})

const role = computed<'KASIR' | 'PEMILIK'>(() => {
  const currentRole = globalUser.value.role?.toUpperCase()
...
const menuGroups = computed(() => {
  const groups: any[] = [
    {
      label: 'Halaman Utama',
      items: [
        { label: 'Ringkasan', to: role.value === 'PEMILIK' ? '/owner/dashboard' : '/kasir/dashboard', icon: resolveComponent('LucideLayoutGrid') },
        { label: 'Kasir (POS)', to: role.value === 'PEMILIK' ? '/owner/pos' : '/kasir/pos', icon: resolveComponent('LucideMonitorSmartphone') },
      ],
    },
    {
  ...
  if (role.value === 'PEMILIK') {
    groups.push({
      ...
      items: [
        { label: 'Laporan Penjualan', to: '/owner/laporan', icon: resolveComponent('LucideBarChartBig') },
        { label: 'Pengaturan Kedai', to: '/owner/pengaturan', icon: resolveComponent('LucideSettings') },
      ],
    })
  }
...
const activeTo = computed<string | null>(() => {
  ...
  const matches = allPaths.filter((to) => route.path === to || route.path.startsWith(to + '/'))
