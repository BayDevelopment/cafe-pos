...
<template>
  <div
    class="h-screen bg-[#1c1410] text-[#2b1b12] flex font-sans overflow-hidden"
  >
    <!-- COMPONENTS SIDEBAR -->
    ...
    <!-- KONTEN UTAMA -->
    <div
      class="flex-1 flex flex-col min-w-0 bg-[#f8f5ee] rounded-l-3xl md:rounded-none overflow-hidden shadow-2xl"
    >
      ...
      >
        <div class="flex items-center gap-3 min-w-0">
          ...
          <span
            class="md:hidden font-bold text-[#2b1b12] text-sm tracking-widest truncate"
          >
            KEDAI KOPI POS
          </span>
          <span
            class="hidden md:inline text-xs text-[#8A7A68] tracking-wide truncate"
          >
            <template v-if="pendingUser">Memuat data pengguna...</template>
            <template v-else
              >Selamat datang kembali, {{ firstName }} 👋</template
            >
          </span>
        </div>
        ...
        <div ref="profileMenuRef" class="relative flex-shrink-0">
          <div
            v-if="pendingUser"
            class="flex items-center gap-2 md:gap-3 pl-1.5 md:pl-2 pr-1.5 md:pr-3 py-1.5"
          >
            <div
              class="w-9 h-9 rounded-full bg-[#2b1b12]/10 animate-pulse flex-shrink-0"
            ></div>
            <div class="hidden sm:block space-y-1.5">
              <div
                class="h-3 w-24 rounded-full bg-[#2b1b12]/10 animate-pulse"
              ></div>
              <div
                class="h-2.5 w-32 rounded-full bg-[#2b1b12]/10 animate-pulse"
              ></div>
            </div>
          </div>
          ...
          >
            ...
            <div class="hidden sm:block text-left leading-tight max-w-[160px]">
              <p class="text-sm font-semibold text-[#2b1b12] truncate">
                {{ fullName }}
              </p>
      ...
      <main class="flex-1 overflow-y-auto flex flex-col">
        ...
        >
          <span
            >© {{ currentYear }} Kedai Kopi POS. Seluruh hak cipta
            dilindungi.</span
          >
