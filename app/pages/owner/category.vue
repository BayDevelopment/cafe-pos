...
<template>
  <div class="p-4 md:p-10 max-w-6xl mx-auto space-y-6 md:space-y-8 font-poppins relative">
    ...
    <header
      ...
      <button v-if="isOwner" type="button" class="btn-stamp px-4 py-2.5 text-xs w-full md:w-auto rounded-lg font-semibold"
        ...
        <table class="w-full text-sm text-left min-w-[480px]">
          <thead>
            <tr class="border-b border-[#2b1b12]/10 bg-[#f4eee3]/80">
              ...
              <!-- Kolom Aksi hanya ditampilkan jika role adalah PEMILIK -->
              <th v-if="isOwner" scope="col" class="label-xs text-center text-[#8A7A68] px-6 py-4 font-semibold">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-[#2b1b12]/5">
            <tr v-for="category in categories" :key="category.id" class="hover:bg-[#f4eee3]/60 transition-colors group">
              ...
              <!-- Kolom Aksi (Edit & Hapus) hanya untuk PEMILIK -->
    ...
    <Teleport to="body">
      <div v-if="categoryToDelete && isOwner"
        class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
...
function openCreateForm() {
  if (!isOwner.value) {
    triggerAlert('Akses ditolak. Hanya Pemilik yang dapat menambah kategori.', 'error')
    return
  }
...
function openEditForm(category) {
  if (!isOwner.value) {
    triggerAlert('Akses ditolak. Hanya Pemilik yang dapat mengubah kategori.', 'error')
    return
  }
...
function confirmDelete(category) {
  if (!isOwner.value) {
    triggerAlert('Akses ditolak. Hanya Pemilik yang dapat menghapus kategori.', 'error')
    return
  }
