...
<template>
    <div class="stage min-h-screen relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-7xl mx-auto p-5 md:p-12 space-y-8">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-5">
                        <div>
                            <div class="flex items-center gap-2 mb-3">
                                <span class="mono label-xs px-2.5 py-1 rounded-full bg-[#b8763c] text-[#faf6ee]">Guest Menu</span>
                            ...
                            <div class="flex items-center gap-2.5">
                                ...
                                <h1 class="display text-2xl sm:text-3xl font-bold text-[#2b1b12] tracking-tight">KEDAI KOPI</h1>
                            </div>
                            <p class="mono text-xs text-[#8A7A68] mt-2">Pilih menu, tambahkan ke keranjang, lalu kirim pesanan ke kasir</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="sticky top-0 z-30 -mx-5 px-5 py-3.5 bg-[#1c1410]/90 backdrop-blur-md border-y border-[#2b1b12]/60 shadow-xl transition-all">
                <div class="max-w-7xl mx-auto space-y-3">
                    ...
                    <!-- FILTER KATEGORI -->
                    <div class="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar sm:flex-wrap sm:overflow-visible">
                        <button type="button" class="category-pill mono shrink-0"
                ...
                <!-- KATALOG PRODUK -->
                 <div class="xl:col-span-2 space-y-8">
                    ...
                    <div v-else class="space-y-8">
                        <div v-if="filteredGroups.length === 0" class="ticket-card p-10 sm:p-12 text-center">
                            <p class="mono text-xs text-[#8A7A68]">
                                {{ query ? `Tidak ada menu yang cocok dengan "${query}".` : 'Tidak ada produk di kategori ini.' }}
                            </p>
                        </div>
                        ...
                        <div v-for="group in filteredGroups" :key="group.name" class="space-y-4">
                          ...
                          <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
                                <div v-for="product in group.items" :key="product.id"
                                    ...
                                    <!-- BADGE DISKON KONVERSI KE PERSEN (misal: 16% OFF) -->
                                    <div v-if="getDiscountPercent(product) > 0 && product.stock > 0"
                                    ...
                                    <div class="h-28 sm:h-32 bg-[#f4eee3] flex items-center justify-center border-b border-[#2b1b12]/10">
                                        <img v-if="product.image" :src="product.image" :alt="product.name"
                                            class="w-full h-full object-cover" />
                                        <svg v-else width="30" height="30" viewBox="0 0 34 34" fill="none"
                                            aria-hidden="true">
                                            <path d="M6 13h18v7a7 7 0 0 1-7 7h-4a7 7 0 0 1-7-7v-7Z" stroke="#C9B8A2"
                                                stroke-width="1.6" />
                                            <path d="M24 15h2.5a3.5 3.5 0 0 1 0 7H24" stroke="#C9B8A2"
                                                stroke-width="1.6" />
                                        </svg>
                                    </div>
                                    ...
                                    <div class="p-4 sm:p-5 flex flex-col justify-between flex-1">
                                        <div>
                                            <h3 class="display text-sm sm:text-base font-bold text-[#2b1b12] leading-snug mb-2">
                                                {{ product.name }}
                                            </h3>

                                            <!-- HARGA SETELAH POTONGAN & HARGA ASLI (CORET) -->
                                            <div class="flex items-baseline gap-2 flex-wrap">
                                                <p class="display text-base sm:text-lg font-bold text-[#b8763c]">
                                                    Rp {{ getFinalPrice(product).toLocaleString('id-ID') }}
                                                </p>
                                                <p v-if="Number(product.discount) > 0" class="mono text-xs line-through text-[#8A7A68]">
                                                    Rp {{ Number(product.price).toLocaleString('id-ID') }}
                                                </p>
                                            </div>
                                        </div>

                                        <div class="mt-4 pt-3 border-t border-[#2b1b12]/10 flex items-center justify-between gap-2">
                                            <span class="mono text-[0.65rem]"
                                                :class="product.stock <= 0 ? 'text-[#9b3a2e] font-semibold' : 'text-[#8A7A68]'">
                                                {{ product.stock <= 0 ? 'HABIS' : `STOK: ${product.stock}` }}
                                            </span>

                                            <button @click="addToCart(product)"
                                                :disabled="product.stock <= 0 || cartQtyOf(product.id) >= product.stock"
                    ...
                    <div class="ticket-card p-4 sm:p-5 md:p-6 space-y-5">
                        ...
                        <template v-if="!activeOrderId">
                            <div class="flex items-center justify-between">
                                <h3 class="display text-base font-bold text-[#2b1b12]">Keranjang</h3>
                                <span class="mono label-xs text-[#8A7A68]">{{ cart.length }} jenis</span>
                            </div>

                            <div v-if="cart.length === 0" class="py-8 text-center">
                                <p class="mono text-xs text-[#8A7A68]">Keranjang masih kosong.<br />Klik "+ Tambah" pada menu.</p>
                            </div>
                            ...
                            <div v-else class="space-y-3 max-h-80 overflow-y-auto pr-1">
                                <div v-for="item in cart" :key="item.productId"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="mono text-xs font-semibold text-[#2b1b12] truncate">{{ item.name }}</p>
                                        <div class="flex items-center gap-1.5">
                                            <p class="mono text-[0.7rem] text-[#8A7A68]">
                                                Rp {{ item.price.toLocaleString('id-ID') }}
                                            </p>
                                            <span v-if="item.discountPercent > 0" class="mono text-[0.65rem] text-[#9b3a2e] font-bold">
                                    ...
                                    <div class="flex items-center gap-1.5 shrink-0">
                                        ...
                                        <span class="mono text-xs w-5 text-center">{{ item.quantity }}</span>
                                        <button @click="incrementQty(item)" :disabled="item.quantity >= item.stock"
                                            class="qty-btn">+</button>
                                    </div>
                                </div>
                            </div>

                            <div v-if="cart.length > 0" class="pt-3 border-t border-[#2b1b12]/10 space-y-3">
                                <div class="flex justify-between mono text-sm font-bold text-[#2b1b12]">
                                    <span>Subtotal</span>
                                    <span>Rp {{ cartSubtotal.toLocaleString('id-ID') }}</span>
                                </div>

                                <input v-model="customerName" type="text" placeholder="Nama Anda (opsional)"
                                    class="field w-full text-xs" maxlength="100" />
                                <textarea v-model="note" placeholder="Catatan (opsional), misal: less sugar"
                                    class="field w-full text-xs" rows="2" maxlength="300"></textarea>

                                <p v-if="submitError" class="mono text-[0.7rem] text-[#9b3a2e]">{{ submitError }}</p>

                                <button @click="submitOrder" :disabled="submitting" class="btn-submit mono w-full">
                        ...
                        <template v-else>
                            <div class="flex items-center justify-between">
                                <h3 class="display text-base font-bold text-[#2b1b12]">Status Pesanan</h3>
                                <span :class="statusBadgeClass" class="mono label-xs px-2 py-0.5 rounded-full">
                                    {{ statusLabel }}
                                </span>
                            </div>
                            ...
                            <div v-if="orderStatus" class="space-y-3">
                                <div class="space-y-2 max-h-64 overflow-y-auto pr-1">
                                    <div v-for="item in orderStatus.items" :key="item.id"
                                        class="flex items-center justify-between gap-2 pb-2 border-b border-[#2b1b12]/10 last:border-0">
                                        <p class="mono text-xs text-[#2b1b12]">{{ item.productName }} <span
                                                class="text-[#8A7A68]">×{{ item.quantity }}</span></p>
                                        <p class="mono text-xs text-[#2b1b12]">Rp {{ (Number(item.price) *
                                ...
                                <div class="pt-3 border-t border-[#2b1b12]/10 space-y-1.5">
                                    <div class="flex justify-between mono text-xs text-[#8A7A68]">
                                        <span>Subtotal</span>
                                        <span>Rp {{ requestSubtotal.toLocaleString('id-ID') }}</span>
                                    </div>

                                    <div v-if="orderStatus.order && Number(orderStatus.order.discount) > 0"
                                        class="flex justify-between mono text-xs text-[#2f7a46]">
                                        <span>Diskon</span>
                                        <span>− Rp {{ Number(orderStatus.order.discount).toLocaleString('id-ID') }}</span>
                                    </div>

                                    <div class="flex justify-between mono text-sm font-bold text-[#2b1b12] pt-1">
                                        <span>Total</span>
                                        <span>
                                            Rp {{ (orderStatus.order ? Number(orderStatus.order.totalAmount) :
                                                requestSubtotal).toLocaleString('id-ID') }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <button v-if="orderStatus.status !== 'PENDING'" @click="resetOrder"
...
function triggerAlert(msg, type = 'success') {
    ...
    alertTimeout = setTimeout(() => { showAlert.value = false }, 3000)
}
...
const persistCart = () => {
    if (process.client) {
        sessionStorage.setItem(CART_STORAGE_KEY, JSON.stringify({
            cart: cart.value, customerName: customerName.value, note: note.value
        }))
    }
}
...
const statusLabel = computed(() => {
    switch (orderStatus.value?.status) {
        ...
        default: return 'Menunggu Kasir'
    }
})

const statusBadgeClass = computed(() => {
    switch (orderStatus.value?.status) {
        case 'ACCEPTED': return 'bg-[#2f7a46]/10 text-[#2f7a46]'
        case 'REJECTED': return 'bg-[#9b3a2e]/10 text-[#9b3a2e]'
        default: return 'bg-[#b8763c]/10 text-[#b8763c]'
    }
})

const statusHelperText = computed(() => {
    switch (orderStatus.value?.status) {
        case 'ACCEPTED': return 'Pesanan Anda sedang disiapkan. Silakan tunggu di kasir.'
        case 'REJECTED': return 'Mohon maaf, pesanan tidak dapat diproses. Silakan hubungi kasir.'
        default: return 'Pesanan Anda sedang menunggu konfirmasi kasir...'
    }
})

watch(() => orderStatus.value?.status, (newStatus, oldStatus) => {
...
const submitOrder = async () => {
    ...
    try {
        ...
        triggerAlert('Pesanan Anda berhasil dikirim! Silakan menuju kasir.', 'success')
    } catch (err) {
        const msg = err?.data?.statusMessage || 'Gagal mengirim pesanan. Coba lagi.'
