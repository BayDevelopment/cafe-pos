<template>
  <div
    class="p-4 md:p-10 max-w-2xl mx-auto space-y-6 font-sans print:p-0 print:m-0 print:max-w-none"
  >
    <!-- TOMBOL AKSI / NAVIGASI (HILANG SAAT DICETAK) -->
    <div
      class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 print:hidden"
    >
      <NuxtLink
        to="/kasir/pos"
        class="mono text-xs text-[#8A7A68] hover:text-[#2b1b12] inline-flex items-center gap-1 self-start py-1"
      >
        ← Kembali ke POS
      </NuxtLink>
      ...
      <div class="grid grid-cols-2 gap-2 w-full sm:flex sm:w-auto sm:gap-2">
        <button
          type="button"
          class="btn-stamp mono px-3 sm:px-4 py-2.5 text-xs w-full sm:w-auto whitespace-nowrap flex items-center justify-center gap-2"
          @click="printReceipt"
        >
        ...
        <NuxtLink
          to="/kasir/pos"
          class="btn-stamp btn-secondary mono px-3 sm:px-4 py-2.5 text-xs w-full sm:w-auto text-center whitespace-nowrap"
        >
          ＋ TRANSAKSI BARU
        </NuxtLink>
      </div>
    ...
    <div
      v-if="pending"
      class="ticket-card p-10 text-center mono text-xs text-[#8A7A68] print:hidden"
    >
    ...
    <div
      ...
      class="ticket-card p-10 text-center space-y-3 print:hidden"
    >
      <p class="mono text-xs text-[#9b3a2e]">
        {{
          fetchError?.data?.statusMessage ||
          "Transaksi tidak ditemukan atau gagal memuat data."
        }}
      </p>
      <NuxtLink
        to="/kasir/pos"
    ...
    <div
      v-else
      class="receipt-body ticket-card p-6 md:p-8 space-y-5 print:p-0 print:shadow-none print:border-none"
    >
      <!-- HEADER STRUK -->
      <div
      ...
      <!-- METADATA TRANSAKSI -->
      <div
        class="mono text-[0.7rem] border-b border-dashed border-[#2b1b12]/30 pb-3 space-y-1"
      >
        <div class="flex justify-between">
          <span class="text-[#8A7A68] print:text-black">No. Struk:</span>
          <span class="font-bold text-[#2b1b12]"
            >#{{ formatInvoiceNo(order.id) }}</span
          >
        </div>
        <div class="flex justify-between">
        ...
        <div class="flex justify-between">
          ...
          <span class="font-bold text-[#2b1b12] uppercase">{{
            order.paymentMethod
          }}</span>
        </div>
      </div>
      ...
      <div class="space-y-2">
        <div
          class="flex justify-between items-center mono text-[0.68rem] text-[#8A7A68] print:text-black border-b border-[#2b1b12]/20 pb-1 font-bold"
        >
          <span>ITEM</span>
          <span>TOTAL</span>
        </div>
        ...
        <div class="space-y-2">
          ...
          >
            <!-- Nama Produk -->
            <p class="font-bold text-[#2b1b12] leading-tight">
              {{ item.product?.name || "Produk" }}
            </p>

            <!-- Rincian Qty x Harga & Subtotal -->
            <div
              class="flex justify-between items-center text-[0.68rem] text-[#8A7A68] print:text-black mt-0.5"
            >
              <span
      ...
      <!-- TOTAL & PEMBAYARAN -->
      <div
        class="border-t border-dashed border-[#2b1b12]/30 pt-3 space-y-1.5 mono text-xs"
      >
        <div
          v-if="Number(order.discount) > 0"
          class="flex justify-between text-[#8A7A68] print:text-black"
        >
          <span>Diskon</span>
          <span>- {{ formatCurrency(order.discount) }}</span>
        </div>
        <div
          class="flex justify-between items-center text-sm font-bold text-[#2b1b12] pt-1 border-t border-[#2b1b12]/20"
        >
          <span>TOTAL</span>
          <span
            class="display text-base font-bold text-[#2b1b12] print:text-black"
            >{{ formatCurrency(order.totalAmount) }}</span
          >
        </div>
        <div
          v-if="order.status"
          class="flex justify-between text-[0.68rem] text-[#8A7A68] print:text-black pt-1"
        >
          <span>Status</span>
          <span class="font-bold text-[#2b1b12] print:text-black">{{
            order.status
      ...
      <div
        v-if="order.note"
        class="bg-[#f4eee3] print:bg-transparent p-2 rounded text-[0.68rem] mono text-[#8A7A68] print:text-black border border-[#2b1b12]/20"
      >
      ...
      <!-- FOOTER STRUK -->
      <div
        class="text-center pt-3 border-t border-dashed border-[#2b1b12]/30 space-y-1"
      >
        <p class="display text-xs font-bold text-[#2b1b12]">Terima Kasih!</p>
        <p class="mono text-[0.6rem] text-[#8A7A68] print:text-black">
...
const {
  ...
  pending,
  error: fetchError,
} = await useFetch(`/api/order/${route.params.id}`, {
  headers: token.value ? { Authorization: `Bearer ${token.value}` } : {},
});
