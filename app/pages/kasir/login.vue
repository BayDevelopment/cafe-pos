          ...
          <div class="ticket">
            ...
            <div class="px-8 pt-8 pb-6 text-center">
              ...
              <p class="mono text-xs text-[#8A7A68]">
                Masuk menggunakan kredensial kasir Anda
              </p>
            </div>
            ...
            <div class="px-8 pt-6 pb-8">
              ...
              >
                <span>✓ DITERIMA — {{ successMessage }}</span>
              </div>

              <!-- STEMPEL ALERT ERROR (MERAH) -->
              <div
                v-if="errorMessage"
                role="alert"
                aria-live="assertive"
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    placeholder="kasir@kedaikopi.com"
                    :class="['field', errorMessage ? 'field-error' : '']"
                    @input="errorMessage = ''"
  ...
  try {
    ...
    if (res && res.success) {
      ...
      if (normalizedRole !== "KASIR") {
        errorMessage.value = `Akses ditolak. Peran akun Anda adalah (${rawRole}), bukan KASIR.`;
        return;
      }

      // SIMPAN STATE USER KE USEAUTH
      setUser(res.user);

      // 1. Tampilkan alert sukses
      successMessage.value = "LOGIN BERHASIL! MENGALIHKAN…";

      // 2. Jeda singkat 400ms agar UI responsif
      await new Promise((resolve) => setTimeout(resolve, 400));

      // 3. Pindah halaman ke dashboard kasir
      await navigateTo("/kasir/dashboard");
    } else {
      errorMessage.value = res?.message || "Login gagal.";
    }
  } catch (error: any) {
    // 1. Ambil pesan spesifik dari JSON backend (jika ada)
    ...
    if (statusCode === 401) {
      // Jika 401 Unauthorized, gunakan pesan dari server ATAU gunakan teks default salah password
      errorMessage.value = serverMessage || "Email atau kata sandi salah!";
    } else {
      // Jika error lain (misal 500 Internal Server Error, masalah jaringan, dll)
      errorMessage.value = serverMessage || "Terjadi kesalahan pada server!";
    }
  } finally {
    isLoading.value = false;
  }
...
.field-error {
  ...
  background-color: rgba(155, 58, 46, 0.12) !important;
  color: #9b3a2e !important;
}

.field-error:focus {
  outline: 2px solid #9b3a2e !important;
}
