...
// --- Pengaturan Toko (untuk header struk: logo, nama toko, alamat, no HP) ---
...
// --- Deteksi Produk Baru (disamakan dengan versi PEMILIK) ---
...
// --- Formatters & Item Helpers (disamakan dengan versi PEMILIK) ---
...
<template>
  <div class="p-4 sm:p-6 md:p-10 max-w-6xl mx-auto space-y-6 md:space-y-8 font-sans relative">
    ...
    <header class="ticket-card p-5 md:p-6 space-y-5">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex items-center gap-2 self-start md:self-auto">
          ...
          <span class="mono label-xs text-[#8A7A68]">{{ pending ? "MENYINKRONKAN..." : "LIVE · TERSAMBUNG" }}</span>
        </div>
      </div>
            ...
            <div class="flex items-start justify-between gap-2">
              <div>
                ...
                <p class="mono text-[0.7rem] text-[#8A7A68] mt-0.5">
                  Pelanggan:
                  <span class="font-semibold text-[#2b1b12]">{{ trx.customerName || "Pelanggan Umum" }}</span>
                </p>
              </div>
    ...
    <!-- MODAL CONFIRM DELETE (PEMILIK ONLY) -->
    <Teleport to="body">
        ...
        <div class="ticket-card w-full max-w-sm p-6 space-y-4">
          <div class="text-center space-y-2">
            ...
            <p class="mono text-xs text-[#8A7A68]">
              ...
              <span class="font-bold text-[#2b1b12]">#{{ formatInvoiceNo(trxToDelete.invoiceNo || trxToDelete.id) }}</span>? Tindakan ini tidak dapat dibatalkan.
            </p>
          </div>
      ...
      <div v-if="selectedTrx"
        ...
        <div class="ticket-card receipt-card w-full max-w-md p-6 pb-8 space-y-5">
          <div class="text-center border-b border-dashed border-[#2b1b12]/20 pb-4">
            <!-- LOGO TOKO -->
            <img v-if="shopSettings.logo_url"
            ...
            <!-- ALAMAT TOKO -->
            <p v-if="shopSettings.address" class="mono text-[0.68rem] text-[#8A7A68] mt-0.5">
              {{ shopSettings.address }}
            </p>

            <!-- NO TELEPON TOKO -->
            <p v-if="shopSettings.phone" class="mono text-[0.68rem] text-[#8A7A68]">
              {{ shopSettings.phone }}
            </p>
            ...
            <div class="mt-3 pt-2 border-t border-dotted border-[#2b1b12]/10 flex justify-between text-xs mono text-[#2b1b12]">
              <div class="text-left">
              ...
              <div class="text-right">
                ...
                <span class="font-bold">{{ selectedTrx.cashier?.name || selectedTrx.cashierName || "Kasir" }}</span>
              </div>
            </div>
          </div>
