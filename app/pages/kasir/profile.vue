  ...
  <div class="p-6 md:p-10 max-w-7xl mx-auto space-y-8 font-sans relative">
    ...
    <Transition name="slide-fade">
      <div
        ...
        :class="
          alertType === 'success'
            ? 'bg-[#1c1410] text-[#faf6ee] border-[#c9793f]'
    ...
    <!-- HEADER / KARTU PROFIL UTAMA (SATU BLOK, TANPA DUPLIKASI SKELETON) -->
    <div
      class="ticket-card p-6 md:p-8 relative overflow-hidden bg-[#faf6ee] text-[#1c1410]"
    >
      <div
      ...
      >
        <div class="flex items-center gap-5">
          <div class="relative flex-shrink-0">
            ...
            <div
              v-else
              class="skeleton w-16 h-16 md:w-20 md:h-20 rounded-2xl"
            ></div>

            <span
              v-if="!showSkeleton"
              class="absolute -bottom-1 -right-1 block h-4 w-4 rounded-full ring-2 ring-[#faf6ee]"
          ...
          <div>
            <div class="flex items-center gap-2 mb-1">
              <span
                v-if="!showSkeleton"
                class="mono label-xs px-2 py-0.5 rounded-md bg-[#c9793f]/10 text-[#c9793f] font-bold"
              >
                {{ user?.role || "KASIR" }}
              </span>
              <div v-else class="skeleton h-4 w-16 rounded-md"></div>
            </div>

            <h1
              v-if="!showSkeleton"
              class="display text-2xl md:text-3xl text-[#1c1410] tracking-tight font-bold"
            >
              {{ user?.name }}
            </h1>
            <div v-else class="skeleton h-7 w-40 md:w-48 rounded mt-0.5"></div>

            <p v-if="!showSkeleton" class="mono text-xs text-[#1c1410]/70 mt-1">
              {{ user?.email }}
            </p>
            <div v-else class="skeleton h-3 w-32 rounded mt-2"></div>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <template v-if="showSkeleton">
        <div
          class="ticket-card p-6 md:p-8 bg-[#faf6ee] md:col-span-2 space-y-6"
        >
          <div class="space-y-2">
          ...
          <div
            class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-[#1c1410]/10"
          >
            <div class="skeleton h-11 rounded-xl"></div>
            <div class="skeleton h-11 rounded-xl"></div>
      ...
      <template v-else>
        ...
        >
          <div>
            <h3
              class="display text-lg text-[#1c1410] font-bold flex items-center gap-2"
            >
              <LucideUserCheck class="w-5 h-5 text-[#c9793f]" />
              <span>Pengaturan & Keamanan Akun</span>
            </h3>
            <p class="mono text-xs text-[#1c1410]/60 mt-1">
              Perbarui informasi identitas diri dan kata sandi akun Anda.
            </p>
          </div>
          ...
          <form @submit.prevent="updateProfile" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="space-y-1.5">
                <label class="mono label-xs text-[#1c1410]/70 block"
                  >NAMA LENGKAP</label
                >
                <input
                  v-model="form.name"
              ...
              <div class="space-y-1.5">
                <div class="flex justify-between items-center">
                  ...
                  <span
                    ...
                    class="text-[10px] text-amber-700 mono font-semibold"
                    >Terkunci (Sudah Terverifikasi)</span
                  >
                  <span
                    v-else
                    class="text-[10px] text-emerald-700 mono font-semibold"
                    >Belum Verifikasi (Dapat Diubah)</span
                  >
                </div>
                <input
                  v-model="form.email"
                  ...
                  :class="
                    isEmailDisabled
                      ? 'opacity-60 cursor-not-allowed bg-[#1c1410]/5 border-transparent'
                      : 'bg-white/60'
                  "
            ...
            >
              <div class="space-y-1.5 sm:col-span-2">
                ...
                >
                <p class="mono text-[10px] text-[#1c1410]/50 -mt-1 mb-1">
                  Kosongkan semua kolom di bawah jika tidak ingin mengubah
                  password. Mengganti password akan mengakhiri sesi ini dan Anda
                  perlu login ulang.
                </p>
              </div>
            ...
            <div class="pt-2 flex justify-end">
              ...
              >
                ...
                <LucideSave
                  v-else
                  class="w-4 h-4 text-current transition-colors duration-200"
                />
                <span>{{
                  loadingUpdate ? "MENGUBAH DATA..." : "SIMPAN PERUBAHAN"
                }}</span>
              </button>
            </div>
          </form>
        ...
        <div class="ticket-card p-6 md:p-8 bg-[#faf6ee] space-y-6 self-start">
          <div>
            <h3
              class="display text-lg text-[#1c1410] font-bold flex items-center gap-2 mb-4"
            >
            ...
            <div class="space-y-4">
              <div
                class="p-4 rounded-xl border border-[#1c1410]/10 bg-white/40 space-y-1"
              >
                <span class="mono label-xs text-[#1c1410]/60"
                  >HAK AKSES SISTEM</span
                >
                <p class="display text-base font-bold text-[#1c1410]">
                  {{
                    user?.role === "PEMILIK"
                      ? "Pemilik Toko (Full Control)"
                      : "Kasir / POS Operator"
                  }}
                </p>
              ...
              <div
                class="p-4 rounded-xl border border-[#1c1410]/10 bg-white/40 space-y-2"
              >
                <span class="mono label-xs text-[#1c1410]/60"
                ...
                <p
                  ...
                  class="mono text-[10px] text-rose-600 mt-1"
                >
                  {{ verificationError }}
                </p>
              </div>
          ...
          <div
            class="p-4 rounded-2xl bg-[#c9793f]/5 border border-[#c9793f]/20"
          >
            <div class="flex items-start gap-3">
              <LucideInfo class="w-5 h-5 text-[#c9793f] flex-shrink-0 mt-0.5" />
              <p class="mono text-[0.7rem] text-[#1c1410]/70 leading-relaxed">
                Pastikan menjaga kerahasiaan kata sandi akun Anda demi keamanan
                transaksi kedai.
              </p>
            </div>
          </div>
...
const showSkeleton = computed(() => pending.value && !user.value);
...
const updateProfile = async () => {
  ...
  if (isEmailDisabled.value && form.email !== user.value?.email) {
    triggerAlert(
      "Email tidak dapat diubah karena sudah terverifikasi.",
      "error",
    );
    return;
  }
  ...
  const wantsPasswordChange =
    !!form.newPassword || !!form.oldPassword || !!form.confirmPassword;
  ...
  if (wantsPasswordChange) {
    ...
    if (!form.newPassword || form.newPassword.length < MIN_PASSWORD_LEN) {
      triggerAlert(
        `Password baru minimal harus ${MIN_PASSWORD_LEN} karakter.`,
        "error",
      );
      loadingUpdate.value = false;
      return;
  ...
  try {
    ...
    if (res?.success) {
      ...
      triggerAlert("Profil berhasil diperbarui!", "success");
    ...
    triggerAlert(
      err?.data?.message || err.message || "Gagal memperbarui profil.",
      "error",
    );
  } finally {
    loadingUpdate.value = false;
