  ...
  if (existingIndex > -1) {
    ...
    if (currentQty < product.stock) {
      ...
      Swal.fire({
        ...
        position: 'top-end',
        icon: 'warning',
        title: 'Kuantitas melebihi stok tersedia!',
...
// --- SILENT PRINT (IFRAME) ---
function printReceiptSilently(orderId) {
  ...
  iframe.src = `/kasir/order/${orderId}`
...
async function checkout() {
  ...
  try {
    ...
    if (res && res.success && res.orderId) {
      ...
      // 3. Print Struk Otomatis
      printReceiptSilently(orderId)

      // 4. Reset Form & Refresh Data Stok
      cart.value = []
      customerName.value = ''
      orderNote.value = ''
      discount.value = 0
